...
class ProcessDispatch:
    ...
    def process_dispatch_add(self,production_code,user_name):
        """生产管理-工序派工"""
        relative_url = "admin-api/mes/pro/dispatch"
        data_lists = self.process_dispatch_payload_list_get(production_code)
        # 获取当前时间并格式化
        current_time = datetime.now()
        formatted_time = current_time.strftime("%Y-%m-%d %H:%M:%S")

        # 处理 list 数据，修改dispatchQuantity的值
        for item in data_lists:
            dept_name = item["deptName"]
            dept_list = [int(x) for x in dept_name.split(',')]
            item["dispatchQuantity"] = item["disQuantity"]
            item["staffName"] = user_name
            item["staffId"] = self.worker_info_get(user_name)
            # TODO创建人
            create_by = "admin"
            # 构建 payload
            payload = {
                "createBy" : create_by,
                "createById" : self.worker_info_get(create_by),
...
#     # 创建实例
#     pdp = ProcessDispatch()
#     pdp.process_dispatch_add("MO202506180001","jwg")
