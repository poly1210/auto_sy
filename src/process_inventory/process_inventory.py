...
class ProcessInventory:
    ...
    def auto_process_inventory_code(self):
        ...
        process_inventory_batch_code = self.api.send_get_direct(relative_url)
        return process_inventory_batch_code

    def process_inventory_payload_get(self, production_code):
        """根据工单编号获取请求负载"""
        relative_url =f"admin-api/pro/recpt/selectListByProWorkorderRecpt?pageNum=1&pageSize=10&workorderCode={production_code}"
    ...
    def process_inventory_add(self, production_code):
        item = self.process_inventory_payload_get(production_code)
        # 后续这个仓库要写成可传入的
